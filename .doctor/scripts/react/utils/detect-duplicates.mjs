...
const SCRIPT_VERSION = createHash('sha256')
  .update(readFileSync(fileURLToPath(import.meta.url), 'utf-8'))
  .digest('hex')
  .slice(0, 12)
...
function findFiles(dir) {
  ...
  try {
    const result = execSync(`git ls-files --cached --others --exclude-standard -- "${dir}"`, {
      encoding: 'utf-8',
    ...
    /* not in git or git not available — fallback */
  }
  ...
  function visit(node) {
    ...
    if (body && ts.isBlock(body) && body.statements.length >= MIN_STMTS) {
      ...
      fns.push({
        ...
        sf,
        paramNames,
        startLine: ln(sf, node.getStart(sf)),
        endLine: ln(sf, node.end),
        stmts: Array.from(body.statements),
        text: body.getText(sf),
      })
    }
...
function describeFn(fn) {
  ...
  // Deep merge
  if (/merge|combine/i.test(fn.name) && /typeof.*object/.test(t))
    extra.push('deep merge combine objects')
  // Event listener
  if (fn.calls.has('listen') || fn.calls.has('addEventListener'))
    extra.push('event listener subscription')
  ...
  if (fn.calls.has('Math.round') && fn.calls.has('Math.max'))
    extra.push('mathematical rounding clamping')
...
function extractHooks(sf, path) {
  ...
  function visit(node) {
    ...
    if (ts.isFunctionDeclaration(node) && node.body) {
      ...
      const p = node.parent
      name =
        ts.isVariableDeclaration(p) && ts.isIdentifier(p.name) ? p.name.text : node.name?.text || ''
    }
...
function detectBlockDupes(allBlocks, funcDupeSet) {
  ...
  for (const r of sorted) {
    if (
      !kept.some(
        (k) => (covers(k.a, r.a) && covers(k.b, r.b)) || (covers(k.a, r.b) && covers(k.b, r.a)),
      )
    )
      kept.push(r)
  }
...
function detectMagicStrings(allStrings) {
  ...
  for (const [value, occ] of byVal) {
    ...
    if (
      /^(div|span|p|a|h[1-6]|ul|li|input|form|label|img|button|section|header|footer|nav|main|aside)$/i.test(
        value,
      )
    )
      continue
    if (
      /^(true|false|null|undefined|none|auto|inherit|block|flex|grid|hidden|visible)$/i.test(value)
    )
      continue
...
function detectHookReuse(data, hooks) {
  ...
  for (const hook of hooks) {
    ...
    const wrapped = [...hook.calls].filter(
      (c) =>
    ...
    for (const fn of data) {
      ...
      if (shared.length > 0) {
        if (
          !res.some(
            (r) => r.hook.name === hook.name && r.fn.file === fn.file && r.fn.name === fn.name,
          )
        )
  ...
  if (verbose) {
    const show = (fn) =>
      fn.text
        ...
        .map((l) => `  │ ${l}`)
        .join('\n')
    console.log(`  ┌─ ${d.a.name} (${d.a.file})`)
    console.log(show(d.a))
    console.log(`  ┌─ ${d.b.name} (${d.b.file})`)
    console.log(show(d.b))
...
for (const d of allFuncDupes) funcDupeSet.add(`${d.a.file}:${d.a.name}|${d.b.file}:${d.b.name}`)
...
const funcDupes = allFuncDupes.filter((d) => {
  if (isPairIgnored(ignore.dupe, d.a.name, d.b.name)) {
    ignored++
    return false
  }
  return true
...
const blockDupes = allBlockDupes.filter((d) => {
  if (isPairIgnored(ignore.block, d.a.parent, d.b.parent)) {
    ignored++
    return false
  }
  return true
...
const typeOverlaps = allTypeOverlaps.filter((d) => {
  if (isPairIgnored(ignore.type, d.a.name, d.b.name)) {
    ignored++
    return false
  }
  return true
...
const hookReuses = allHookReuses.filter((d) => {
  if (isPairIgnored(ignore.hook, d.hook.name, d.fn.name)) {
    ignored++
    return false
  }
  return true
...
if (total === 0) {
  ...
  console.log(
    `  Scanned ${files.length} files, ${allFns.length} functions, ${allTypes.length} types${cacheInfo}${ignoreInfo}`,
  )
} else {
  console.log('')
  console.log(
    `TOTAL: ${total} issue(s) across ${files.length} files, ${allFns.length} functions, ${allTypes.length} types${cacheInfo}${ignoreInfo}`,
  )
  process.exitCode = 1
}
